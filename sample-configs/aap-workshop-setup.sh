...
[[ -e ./config/aap-workshop-setup.ans ]] && . ./config/aap-workshop-setup.ans
...
cat > ./config/aap-workshop-setup.ans <<EO_ANSWERS
...
current_settings () {
    ...
    echo "Vault AAP     UID/PW ... ${AAP_UID} / ${AAP_PW:+"**********"}" 
    ...
    echo "AAP: ${ADDR_AAP} / ${MAC_AAP} / ${HW_AAP} / ${BMC_AAP} / ${NAME_AAP}"
...
prepare_deployment () {
    ...
    ansible-playbook sample-configs/aap-workshop-setup.yml
...
node_menu () {
    ...
    select action in "Bastion" "AAP" "Node1" "Node2" "Node3" "Node4" "Back to Main Menu"
    do
      case ${action}  in
        "Bastion")
...
vault_menu () {
    ...
    select action in "Set Ansible Vault Password" "Set RHSM UID" "Set RHSM Password" "Set AAP UID" "Set AAP Password" "Set vHost Password" "Set oVirt Password" "Set Default BMC Password" "Back to Main Menu"
    do
      case ${action}  in
        ...
        "Set AAP UID")
          read -p "Enter Red Hat Subscription Manager User ID [${AAP_UID}]: " input
          AAP_UID=${input:-$AAP_UID}
          ;;

        "Set AAP Password")
          echo "Enter new password and press Enter"
          read -s -p "Enter AAP password [${AAP_PW:+"**********"}]: " input
          echo ""
          read -s -p "Enter AAP password again [${AAP_PW:+"**********"}]: " input2
          echo ""
          echo ""
...
aap_menu () {
    ...
    do
      case ${action}  in
        ...
        "Set Version")
           select AAP_VERSION in "2.3" "2.2" "2.1"
           do
              case ${AAP_VERSION} in
                "2.3" )
                  break ;;
